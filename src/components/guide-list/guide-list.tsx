...
async function loadChild(parentId: string, entry: ChildEntry): Promise<RenderedChild> {
  const path = parentId ? `${parentId}/${entry.id}` : entry.id;
...
export class GuideList {
  ...
  async componentWillLoad() {
    try {
      const indexPath = this.guideId ? `${this.guideId}/index.json` : 'index.json';
      const res = await fetch(`/assets/guides/${indexPath}`, { cache: 'no-cache' });
